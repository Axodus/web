...
if ( ! function_exists( 'olympus_base_css' ) ) {
	...
	function olympus_base_css() {
		...
		$css->add_property( '--oly-main', $settings['global_color'], $defaults['global_color'] );

		$css->set_selector( ':root' );
		$css->add_property( '--oly-main-hover', $settings['global_color_hover'], $defaults['global_color_hover'] );
		...
		$css->set_selector( '#site-navigation.main-navigation .menu > li > a, .olympus-search-icon > a, .olympus-cart-icon > a' );
		$css->add_property( 'color', $settings['menu_links_color'], $defaults['menu_links_color'] );

		$css->set_selector( '#site-navigation.main-navigation .menu > li > a:hover, .olympus-search-icon > a:hover, .olympus-cart-icon > a:hover' );
		$css->add_property( 'color', $settings['menu_links_color_hover'], $defaults['menu_links_color_hover'] );

		$css->set_selector( 'body .oly-is-sticky' );
		$css->add_property( 'background-color', $settings['sticky_background'], $defaults['sticky_background'] );

		$css->set_selector( 'body .oly-is-sticky .main-navigation:not(.toggled) .menu > li > a, body .oly-is-sticky button.menu-toggle, body .oly-is-sticky .olympus-search-icon > a, body .oly-is-sticky .olympus-cart-icon > a' );
		$css->add_property( 'color', $settings['sticky_links_color'], $defaults['sticky_links_color'] );

		$css->set_selector( 'body .oly-is-sticky .main-navigation:not(.toggled) .menu > li > a:hover, body .oly-is-sticky button.menu-toggle:hover, body .oly-is-sticky .olympus-search-icon > a:hover, body .oly-is-sticky .olympus-cart-icon > a:hover' );
		...
		$css->set_selector( '#site-navigation button.menu-toggle .oly-icon' );
		...
		if ( 'hide-scroll' === $settings['sticky_style'] ) {
			$css->set_selector( '.oly-has-sticky.oly-sticky-scroll-down' );
			$css->add_property( 'transform', 'translate3d(0, -100%, 0)' );
		}

		// If sticky logo
		if ( $settings['sticky_logo'] ) {
			$css->set_selector( '.sticky-logo-link, .site-header.oly-has-sticky-logo.oly-is-sticky .custom-logo-link' );
			$css->add_property( 'display', 'none' );

			$css->set_selector( '.site-header.oly-has-sticky-logo.oly-is-sticky .sticky-logo-link' );
		...
		if ( ! empty( $settings['sticky_logo_width']['desktop'] ) || ! empty( $settings['sticky_logo_width']['tablet'] ) || ! empty( $settings['sticky_logo_width']['mobile'] ) ) {
			$css->set_selector( '.oly-has-sticky .site-branding img' );
			$css->add_property( 'transition', '.3s' );
		}

		if ( ! empty( $settings['sticky_logo_width']['desktop'] ) ) {
			$css->set_selector( '.oly-is-sticky .site-branding img' );
		...
		if ( 'none' !== olympus_get_option( 'search_style' ) ) {
			$css->set_selector( '.olympus-search-icon' );
			$css->add_property( 'position', 'relative' );
			$css->add_property( 'display', 'flex' );

			$css->set_selector( '.olympus-search-icon a' );
			$css->add_property( 'font-size', '15px' );
			$css->add_property( 'padding', '10px 15px' );

			if ( 'dropdown' === olympus_get_option( 'search_style' ) ) {
				$css->set_selector( '.olympus-search-icon .olympus-search-wrapper' );
				...
				$css->add_property( 'background-color', 'var(--oly-light)' );
				$css->add_property( 'box-shadow', '0 2px 6px rgba(0, 0, 0, .1)' );
				$css->add_property( 'transition', 'all .2s' );
				$css->add_property( 'z-index', '9' );

				$css->set_selector( 'body.olympus-search-active .olympus-search-wrapper' );
				$css->add_property( 'visibility', 'visible' );
				$css->add_property( 'opacity', '1' );

				// RTL
				$css->set_selector( 'body.rtl .olympus-search-icon .olympus-search-wrapper' );
				$css->add_property( 'right', 'auto' );
				$css->add_property( 'left', '0' );

				$css->set_selector( '.olympus-search-icon.search-dropdown .olympus-search-wrapper' );
				$css->add_property( 'background-color', $settings['search_dropdown_bg'], $defaults['search_dropdown_bg'] );

				$css->set_selector( '.olympus-search-icon.search-dropdown input[type="search"]' );
				...
				$css->set_selector( '.olympus-search-icon.search-dropdown input[type="search"], .olympus-search-icon.search-dropdown input[type="search"]::placeholder' );
				$css->add_property( 'color', $settings['search_dropdown_input_color'], $defaults['search_dropdown_input_color'] );

				$css->set_selector( '.olympus-search-icon.search-dropdown input[type="search"]:hover' );
				$css->add_property( 'border-color', $settings['search_dropdown_input_border_hover'], $defaults['search_dropdown_input_border_hover'] );

				$css->set_selector( '.olympus-search-icon.search-dropdown input[type="search"]:focus' );
			...
			if ( 'full-screen' === olympus_get_option( 'search_style' ) ) {
				$css->set_selector( '.olympus-search-full-screen' );
				$css->add_property( 'opacity', '0' );
				$css->add_property( 'visibility', 'hidden' );
				...
				$css->set_selector( 'body.olympus-search-active .olympus-search-full-screen' );
				$css->add_property( 'visibility', 'visible' );
				$css->add_property( 'opacity', '1' );

				$css->set_selector( '.olympus-search-full-screen .olympus-search-close' );
				$css->add_property( 'position', 'absolute' );
				...
				$css->set_selector( '.olympus-search-full-screen .olympus-search-wrapper' );
				$css->add_property( 'position', 'absolute' );
				$css->add_property( 'width', '100%' );
				...
				$css->set_selector( '.olympus-search-full-screen .search-container' );
				$css->add_property( 'max-width', '900px' );
				$css->add_property( 'margin', '0 auto' );

				$css->set_selector( '.olympus-search-full-screen .search-text' );
				$css->add_property( 'color', '#e2e2e2' );
				$css->add_property( 'text-align', 'center' );

				$css->set_selector( '.olympus-search-full-screen form' );
				$css->add_property( 'margin-top', '20px' );

				$css->set_selector( '.olympus-search-full-screen input[type="search"]' );
				...
				$css->set_selector( '.olympus-search-full-screen input[type="search"]:focus' );
				$css->add_property( 'border-color', 'var(--oly-main)' );

				// RTL
				$css->set_selector( 'body.rtl .olympus-search-full-screen .olympus-search-close' );
				$css->add_property( 'right', 'auto' );
				...
				$css->set_selector( '.olympus-search-full-screen input[type="search"], .olympus-search-full-screen input[type="search"]::placeholder' );
				$css->add_property( 'color', $settings['search_fullscreen_input_color'], $defaults['search_fullscreen_input_color'] );

				$css->set_selector( '.olympus-search-full-screen input[type="search"]:hover' );
				...
				$css->set_selector( '.olympus-search-full-screen .olympus-search-close:hover' );
			...
			if ( 'slide' === olympus_get_option( 'search_style' ) ) {
				...
				$css->set_selector( '.olympus-search-icon .search-field' );
				$css->add_property( 'min-width', '250px' );
				$css->add_property( 'height', 'auto' );

				$css->set_selector( '.olympus-search-icon .olympus-search-close' );
				$css->add_property( 'position', 'absolute' );
				$css->add_property( 'top', '0' );
				$css->add_property( 'bottom', '0' );
				$css->add_property( 'right', '0' );
				...
				$css->set_selector( '.olympus-search-icon .olympus-search-close svg' );
				$css->add_property( 'top', '0' );

				$css->set_selector( '.olympus-search-icon.search-slide input[type="search"]' );
		...
		// If cart icon
		if ( class_exists( 'WooCommerce' ) && true === olympus_get_option( 'add_cart_icon' ) ) {
			$css->set_selector( '.olympus-cart-icon' );
			$css->add_property( 'position', 'relative' );
			$css->add_property( 'display', 'flex' );

			$css->set_selector( 'a.olympus-cart-link' );
			$css->add_property( 'display', 'flex' );
			$css->add_property( 'align-items', 'center' );
			$css->add_property( 'padding', '10px 15px' );

			$css->set_selector( 'a.olympus-cart-link svg' );
			$css->add_property( 'top', '0' );
			$css->add_property( 'font-size', '15px' );

			$css->set_selector( '.olympus-count' );
			$css->add_property( 'position', 'relative' );
			$css->add_property( 'width', '18px' );
			$css->add_property( 'height', '18px' );
			$css->add_property( 'line-height', '18px' );
			$css->add_property( 'background-color', 'var(--oly-main)' );
			$css->add_property( 'color', 'var(--oly-light)' );
			...
			$css->set_selector( '.olympus-cart-total' );
			$css->add_property( 'line-height', '1' );
			$css->add_property( 'color', '#57bf6d' );
			$css->add_property( 'margin-left', '6px' );

			$css->set_selector( '.olympus-cart-header' );
			$css->add_property( 'display', 'block' );
			$css->add_property( 'background-color', 'var(--oly-light)' );
			...
			$css->set_selector( '.olympus-cart-icon:hover .olympus-cart-header' );
			$css->add_property( 'right', '0' );
			$css->add_property( 'opacity', '1' );
			...
			$css->set_selector( '.olympus-cart-widget .widget,.olympus-cart-widget p' );
			$css->add_property( 'margin', '0' );

			$css->set_selector( '.olympus-cart-widget .woocommerce ul.cart_list' );
			$css->add_property( 'padding-bottom', '20px' );

			$css->set_selector( '.olympus-cart-widget .woocommerce.widget_shopping_cart .cart_list li' );
			$css->add_property( 'padding', '1em 2.3em 1em 5em' );
			$css->add_property( 'border-bottom', '1px solid var(--oly-main-border)' );

			$css->set_selector( '.olympus-cart-widget .woocommerce.widget_shopping_cart .cart_list li:last-child' );
			$css->add_property( 'border-bottom', '0' );

			$css->set_selector( '.olympus-cart-widget .woocommerce.widget_shopping_cart .cart_list li a.remove' );
			$css->add_property( 'top', '0.5em' );
			$css->add_property( 'right', '0' );
			$css->add_property( 'left', 'auto' );

			$css->set_selector( '.olympus-cart-widget .woocommerce ul.cart_list li a.remove' );
			$css->add_property( 'font-size', '16px' );
			$css->add_property( 'line-height', '20px' );

			$css->set_selector( '.olympus-cart-widget .woocommerce ul.cart_list li a' );
			$css->add_property( 'font-size', '13px' );
			$css->add_property( 'font-weight', '400' );
			$css->add_property( 'line-height', '1.3' );

			$css->set_selector( '.olympus-cart-widget .woocommerce ul.cart_list li img' );
			$css->add_property( 'position', 'absolute' );
			$css->add_property( 'left', '0' );
			...
			$css->set_selector( '.olympus-cart-widget p.woocommerce-mini-cart__total.total' );
			$css->add_property( 'padding', '1em 1.5em' );
			$css->add_property( 'margin', '0' );
			$css->add_property( 'text-align', 'center' );

			$css->set_selector( '.olympus-cart-widget .woocommerce .woocommerce-mini-cart__buttons a' );
			$css->add_property( 'display', 'block' );
			$css->add_property( 'margin', '0' );
			$css->add_property( 'text-align', 'center' );

			$css->set_selector( '.olympus-cart-widget .woocommerce .woocommerce-mini-cart__buttons a:first-child' );
			$css->add_property( 'margin-bottom', '6px' );

			$css->set_selector( '.olympus-cart-mobile' );
			$css->add_property( 'display', 'none' );

			$css->set_selector( '.olympus-cart-mobile .oly-icon' );
			$css->add_property( 'margin-right', '5px' );

			$css->set_selector( '.olympus-cart-mobile svg' );
			$css->add_property( 'top', '0' );

			// RTL
			$css->set_selector( 'body.rtl .olympus-count' );
			$css->add_property( 'margin-left', '0' );
			$css->add_property( 'margin-right', '4px' );

			$css->set_selector( 'body.rtl .olympus-cart-total' );
			$css->add_property( 'margin-left', '0' );
			$css->add_property( 'margin-right', '6px' );

			$css->set_selector( 'body.rtl .olympus-cart-widget' );
			$css->add_property( 'float', 'right' );
			$css->add_property( 'right', 'auto' );
			$css->add_property( 'left', '-99999px' );
			$css->add_property( 'text-align', 'right' );

			$css->set_selector( 'body.rtl .olympus-cart-icon:hover .olympus-cart-widget' );
			$css->add_property( 'right', 'auto' );
			$css->add_property( 'left', '0' );

			$css->set_selector( 'body.rtl .olympus-cart-widget .woocommerce.widget_shopping_cart .cart_list li' );
			$css->add_property( 'padding', '1em 5em 1em 2.3em' );

			$css->set_selector( 'body.rtl .olympus-cart-widget .woocommerce.widget_shopping_cart .cart_list li a.remove' );
			$css->add_property( 'right', 'auto' );
			$css->add_property( 'left', '0' );

			$css->set_selector( 'body.rtl .olympus-cart-widget .woocommerce ul.cart_list li img' );
			$css->add_property( 'left', 'auto' );
			$css->add_property( 'right', '0' );

			$css->set_selector( 'body.rtl .olympus-cart-mobile .oly-icon' );
...
if ( ! function_exists( 'olympus_font_css' ) ) {
	...
	function olympus_font_css() {
		...
		$css->set_selector( '.main-navigation .menu > li > a, .menu-toggle, .oly-canvas-link' );
...
function olympus_enqueue_dynamic_css() {
	if ( apply_filters( 'olympus_css_external_file', false ) ) {
		$css = '';
	} elseif ( ! get_option( 'olympus_dynamic_css_output', false )
	|| is_customize_preview()
	|| apply_filters( 'olympus_dynamic_css_skip_cache', false ) ) {
		$css = olympus_get_dynamic_css();
	} else {
		$css = get_option( 'olympus_dynamic_css_output' ) . '/* End cached CSS */';
	}
 ...
 */
function olympus_set_dynamic_css_cache() {
	if ( apply_filters( 'olympus_dynamic_css_skip_cache', false ) ) {
		return;
	}

	$cached_css = get_option( 'olympus_dynamic_css_output', false );
	$cached_version = get_option( 'olympus_dynamic_css_cached_version', '' );

	if ( ! $cached_css || OLY_VERSION !== $cached_version ) {
		$css = olympus_get_dynamic_css();

		update_option( 'olympus_dynamic_css_output', wp_strip_all_tags( $css ) );
		update_option( 'olympus_dynamic_css_cached_version', esc_html( OLY_VERSION ) );
	}
}
add_action( 'init', 'olympus_set_dynamic_css_cache' );

/**
 * Update our CSS cache when done saving Customizer options.
 */
function olympus_update_dynamic_css_cache() {
	if ( apply_filters( 'olympus_dynamic_css_skip_cache', false ) ) {
		return;
	}
...
add_action( 'customize_save_after', 'olympus_update_dynamic_css_cache' );

...
if ( ! class_exists( 'Olympus_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class Olympus_WebFont_Loader {
		...
		public function __construct( $url = '' ) {
			...
			add_action( 'olympus_delete_fonts_folder', array( $this, 'olympus_delete_fonts_folder' ) );
		}
		...
		public function get_url() {
			...
			$font_url = file_exists( $this->get_local_stylesheet_path() ) ? $this->get_local_stylesheet_url() : $this->remote_url;
	
			// If the local file exists, return its URL, with a fallback to the remote URL.
			olympus_update_option( 'olympus_font_url', wp_json_encode( $font_url ) );
		...
		public function get_local_files_from_css() {
			...
			$stored     = get_site_option( 'olympus_downloaded_font_files', array() );
			...
			if ( $change ) {
				...
				update_site_option( 'olympus_downloaded_font_files', $stored );
			}
		...
		public function preload_local_fonts() {
			...
			update_site_option( 'olympus_local_font_files', $local_font );
		...
		public function get_local_stylesheet_filename() {
			return apply_filters( 'olympus_local_font_file_name', 'olympus-fonts' );
		}
		...
		public function set_font_format( $format = 'woff2' ) {
			$this->font_format = apply_filters( 'olympus_local_google_fonts_format', $format );
		}
		...
		public function get_base_path() {
			if ( ! $this->base_path ) {
				$this->base_path = apply_filters( 'olympus_local_fonts_base_path', $this->get_filesystem()->wp_content_dir() );
			}
		...
		public function get_base_url() {
			if ( ! $this->base_url ) {
				$this->base_url = apply_filters( 'olympus_local_fonts_base_url', content_url() );
			}
		...
		public function get_subfolder_name() {
			if ( ! $this->subfolder_name ) {
				$this->subfolder_name = apply_filters( 'olympus_local_fonts_directory_name', 'olympus' );
			}
		...
		public function schedule_cleanup() {
			if ( ! is_multisite() || ( is_multisite() && is_main_site() ) ) {
				if ( ! wp_next_scheduled( 'olympus_delete_fonts_folder' ) && ! wp_installing() ) {
					wp_schedule_event( time(), self::CLEANUP_FREQUENCY, 'olympus_delete_fonts_folder' ); // phpcs:ignore WPThemeReview.PluginTerritory.ForbiddenFunctions.cron_functionality_wp_schedule_event
				}
			}
		}
		...
		public function delete_fonts_folder() {
			...
			olympus_delete_option( 'olympus_font_url' );
			delete_site_option( 'olympus_local_font_files' );
 ...
 * @return object
 */
function olympus_webfont_instance( $font_url = '' ) {
	return new Olympus_WebFont_Loader( $font_url );
}
 ...
 */
function olympus_get_webfont_url( $url, $format = 'woff2' ) {

	// Check if already Google font URL present or not.
	$font_url = olympus_get_option( 'olympus_font_url', false );
	if ( $font_url ) {
		return json_decode( $font_url );
	}

	// Now create font URL if its not present.
	$font = olympus_webfont_instance( $url );
 ...
 * @param string $format The font-format. If you need to support IE, change this to "woff".
 */
function olympus_preload_fonts( $url, $format = 'woff2' ) {

	// Check if cached font files data preset present or not.
	$font_files = get_site_option( 'olympus_local_font_files', false );

	if ( is_array( $font_files ) && ! empty( $font_files ) ) {
		$font_format = apply_filters( 'olympus_local_google_fonts_format', $format );
		foreach ( $font_files as $key => $local_font ) {
			if ( $local_font ) {
 ...
 * @return void
 */
function olympus_regenerate_fonts_folder() {

	check_ajax_referer( 'olympus-regenerate-local-fonts', 'nonce' );

	if ( ! current_user_can( 'edit_theme_options' ) ) {
		wp_send_json_error( 'invalid_permissions' );
	}

	if ( olympus_get_option( 'load_fonts_locally' ) ) {
		$loader = olympus_webfont_instance( '' );
		$flushed = $loader->delete_fonts_folder();
...
add_action( 'wp_ajax_olympus_regenerate_fonts_folder', 'olympus_regenerate_fonts_folder' );

/**
 * Delete the cached partial configs.
 */
function olympus_delete_cached_partials() {
	delete_option( 'olympus_partials_config_cache' );
...
add_action( 'customize_save_after', 'olympus_delete_cached_partials' );
